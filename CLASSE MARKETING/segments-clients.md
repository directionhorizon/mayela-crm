...
**Fichier source** : `mayela-crm.html`, fonction `renderCentreAction()` (~l.2583-2636).
...
Calculé dans la boucle `achats.forEach(a => ...)` (l.2588-2596).
...
**Affichage** : `X / Y consentent` dans la carte « Inactifs » (l.2613).
...
|---|---|---|
| Pool | Clients **inactifs** uniquement | l.2619 |
| Tri | Par **date de dernière activité** (ou date de création) croissante (plus ancien d'abord) | l.2621-2622 |
| Limite | **6 clients** maximum | `.slice(0,6)` |
| Lien WhatsApp | `wa.me/{phone}?text=Bonjour {name} 👋 Ici Mayela...` | l.2627 |
...
**Marquer traité** : cliquer sur ✓ met à jour l'interaction associée (`statut_traitement → 'traite'`) puis recharge le centre d'action (l.2638-2642).
